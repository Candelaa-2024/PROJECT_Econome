...
const UICtrl = (function(){
    ...
    return{
        ...
        addIncomeItem: function(item){
            ...
            div.id = `item-Ron{item.id}`
            div.innerHTML = `
            <h4>${item.description}</h4>
            <div class="item__income">
                <p class="symbol">Ron</p>
...
const App = (function(){
    ...
    const deleteItem = function(e){
        if(e.target.classList.contains('far')){
            console.log('Delete icon clicked', e.target); // Use this to debug
            const id = itemCtrl.getIdNumber(e.target);
            console.log('Item ID to delete', id); // Use this to debug
